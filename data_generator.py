...
class DataGenerator(object):
    ...
    def __init__(self, hdf5_path, target_device, train_house_list, validate_house_list,
                 ...
                 balance_threshold=None, balance_positive=0.5, loss_weight_file=None):
        """Data generator.
        Args:
        ...
        self.loss_weight_file = loss_weight_file
        ...
        (self.train_x, self.train_y, self.loss_weights) = self.read_data(
            self.hf, target_device, train_house_list)

        # Load validation data
        # NOTE read_data will return binarized data if binary_threshold is set to a float
        (self.validate_x, self.validate_y, _) = self.read_data(
    ...
    def read_data(self, hf, target_device, house_list):
        ...
        if len(house_list) == 0:
            ...
            for house in house_list:
                ...
                if self.loss_weight_file is None:
                    loss_weight = np.ones(len(target))
                else:
                    print('loading loss weight file')
                    loss_weight = np.load(self.loss_weight_file)
            ...
            loss_weights = np.concatenate(loss_weights, axis=0)

            return aggregates, targets, loss_weights
    ...
    def _generate_balanced(self):
        ...
        while True:
            ...
            # batch_y_indexes_2d = batch_indexes[:, None] + np.arange(self.seq_len + self.width - 1)
            batch_x = self.train_x[batch_x_indexes_2d]
            batch_y = self.train_y[batch_y_indexes_2d]
            batch_w = self.loss_weights[batch_y_indexes_2d]
            # Normalize input
...
class TestDataGenerator(DataGenerator):
    ...
    def generate_inference(self, house):
        ...
        x, y, _ = self.read_data(self.hf, self.target_device, house_list=[house])
        self.target = y
        self.source = x
