...
def loss_func(output, target, batch_probability,args):
    # logging.info('output shapes: {} and target shapes: {} '.format(output.shape, target.shape))
    assert output.shape == target.shape
    if args.probability is not None:
        # print('----probability is true and use probability----')
        return 1*torch.mean(torch.mul(torch.abs(output - target), batch_probability)) + 0*torch.mean(torch.mul(torch.square(output - target), batch_probability))
        
        
    else:
        # print('----probability is not true and do not use probability----')
...
def binary_metrics(outputs, targets,args):
    outputs = binarize_threshold(outputs, args.wind_seq_len, args.min_len, args.cutoff_threshold)
...
def evaluate(model, generator, data_type, max_iteration, cuda, binary=False, args=None):
    """Evaluate.
    Args:
      model: object.
      generator: object.
...
def forward(model, generate_func, cuda, has_target):
    ...
    for data in generate_func:
        ...
        if has_target:
            ...
            # (batch_x, batch_y, batch_probability) = data
    ...
    if has_target:
        ...
        return outputs, targets#, probability
...
def train(args):
    ...
    # Paths
    hdf5_path = os.path.join(workspace, 'path/')
    ...
    generator = DataGenerator(hdf5_path=hdf5_path,
                              ...
                              wind_seq_len=args.wind_seq_len,
                              min_len=args.min_len,
                              cutoff_threshold=args.cutoff_threshold,
                              binary_threshold_validate=args.binary_threshold_validate)
    ...
    for (batch_x, batch_y, batch_probability) in generator.generate():
    # for (batch_x, batch_y) in generator.generate():
        ...
        if iteration % 100 == 0:
            ...
            va_result_dict = evaluate(model=model,
                                      ...
                                      max_iteration=args.validate_max_iteration,
                                      cuda=cuda,
                                      binary=args.binary_threshold_validate is not None,
                                      args=args)
        ...
        batch_probability = move_data_to_gpu(batch_probability, cuda)
        ...
        if args.binary_threshold is not None:
            ...
            loss = loss_func(output, batch_y, batch_probability,args)
        # Backward
        optimizer.zero_grad()
        loss.backward()
...
def inference(args,house):
    ...
    np.save(workspace+'/outputs/a'+args.inference_model+'_'+house+'_'+'prediction.npy', outputs)
    np.save(workspace+'/outputs/a'+args.inference_model+'_'+house+'_'+'groundtruth.npy', targets)
    np.save(workspace+'/outputs/a'+args.inference_model+'_'+house+'_'+'source.npy', source)
    return mae,sae
