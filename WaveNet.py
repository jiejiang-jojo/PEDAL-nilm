...
def loss_func(output, target, batch_probability,args):
    ...
    if args.probability is not None:
        ...
        return torch.mean(torch.mul(torch.abs(output - target), batch_probability)) + 0*torch.mean(torch.mul(torch.square(output - target), batch_probability))
    else:
...
def binary_metrics(outputs, targets,args):
    ...
    # np.save('validate_output.npy',outputs)
    # np.save('validate_targets.npy',targets)
    # os._exit(0)
